...
- (NSString *)getParticipantID   // VChat
{
    return _hostSession.peerID;
}
...
{
   return  [_hostSession sendDataToAllPeers:data withDataMode:mode error:error];
}
...
- (void)receiveData:(NSData *)data fromPeer:(NSString *)peer inSession:(GKSession *)session context:(void *)context
{
    if(!_isSender)
    {
        ...
        {
            if(self.delegate != nil && [self.delegate respondsToSelector:@selector(btManager:finishedReceivingData:fromPeerName:)])
            {
                [self.delegate btManager:self finishedReceivingData:_receivedData fromPeerName:peer];
            }
